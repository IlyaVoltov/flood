...
def generate_graph():
    graph = html.Div([
        ...
        html.Div([
            dcc.Graph(id = 'map', 
                      figure = cm,
                      hoverData = {'points': [{'location': '8c95756206039444095efcf05f77c9dc'}]})
        ]),
...
def create_barchart(poly_group):
    figure =  {'data' : [go.Bar(x = poly_group.date,
...
def create_plot(hoverData):
    poly_id = hoverData['points'][0]['location']
    poly_group = res.groupby('polygon_id').get_group(poly_id)
    return create_barchart(poly_group)
